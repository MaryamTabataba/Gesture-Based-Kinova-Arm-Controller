...
def do_set_tool(node, set_tool, x, y, z, theta_x, theta_y, theta_z):
    ...
    # remove rclpy.spin_until_future_complete() to stop endless spinning)
    # add callback to allow code to continue
    future.add_done_callback(lambda x: print(f"SetTool returns {future.result()}"))
...
def do_command(msg, node, set_tool):
    # moves the robot to the position related to the command
    node.get_logger().info(f"{msg}")
    if msg == "left": 
        node.get_logger().info("moving left")
        do_set_tool(node, set_tool, 0.2, 0.1, 0.1, 180, 0, 180) 
    elif msg == "right": 
        node.get_logger().info("moving right")
        do_set_tool(node, set_tool, 0.2, -0.1, 0.1, 180, 0, 180) 
    elif msg == "back": 
        node.get_logger().info("moving back")
        do_set_tool(node, set_tool, 0.1, 0.0, 0.1, 180, 0, 180) 
    elif msg == "forward": 
        node.get_logger().info("moving forward")
        do_set_tool(node, set_tool, 0.3, 0.0, 0.1, 180, 0, 180) 
    elif msg == "home": 
        node.get_logger().info("moving home")
        do_set_tool(node, set_tool, 0.2, 0.0, 0.1, 180, 0, 180) 
...
def main():
    ...
    sub = node.create_subscription(String, "/command", lambda msg: do_command(msg.data, node, set_tool), 10)
